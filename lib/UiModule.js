...
class UiModule extends AbstractModule {
  ...
  async init () {
    ...
    server.root.addMiddleware(server.static(this.buildDir))
    server.root.addRoute({ route: '/*splat', handlers: { get: this.servePage('index.html') } })
   ...
   */
  servePage (pageName) {
    return async (req, res, next) => res.sendFile(path.resolve(this.buildDir, pageName))
  }
}
